...
<xsl:stylesheet
    ...
    <xsl:template name="table-cell">
        ...
        <xsl:comment>horizontal-position=<xsl:value-of select="$horizontal-position"/>
        </xsl:comment>
        <xsl:comment>horizontal-count=<xsl:value-of select="$horizontal-count"/>
        </xsl:comment>
        <xsl:comment>vertical-position=<xsl:value-of select="$vertical-position"/>
        </xsl:comment>
        <xsl:comment>vertical-count=<xsl:value-of select="$vertical-count"/>
        </xsl:comment>
        ...
        <table:table-cell office:value-type="string">
            ...
            <xsl:attribute name="table:style-name">
                ...
                <xsl:choose>
                    ...
                    <!-- A3 -->
                    <xsl:when
                            test="$horizontal-position = 1 and $horizontal-count != 1 and $vertical-position = $vertical-count">
                        <xsl:text>A3</xsl:text>
                    </xsl:when>
                    <!-- C3 -->
                    ...
                    <!-- A1 -->
                    <xsl:when
                            test="$horizontal-position = 1 and $horizontal-position != $horizontal-count and $vertical-position = 1">
